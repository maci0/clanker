...
- When proposing an exact-match patch via improve-self, keep every change inside the modifiable surface the gate names; match `old` byte-for-byte against text you have actually been shown — if a file you need to edit is not in context, ask to be shown it first rather than guessing at its contents, and use base64 (`old_b64`/`new_b64`) for quote-heavy descriptors so escaping cannot corrupt the patch. Every accepted change must alter what the program does: no test-only additions, no dead code with no caller.
