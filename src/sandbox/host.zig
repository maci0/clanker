...
fn execPolicyConfig(
    ...
    try s.beginObject();
    // Preserve the tool's own config keys (usually none).
    const parsed = try std.json.parseFromSliceLeaky(std.json.Value, arena, tool_config, .{});
    if (parsed == .object) {
    ...
    try s.write(cfg.agent.exec_pattern_allow);
    try s.endObject();
