...
const commands_by_name = std.StaticStringMap(ParsedCommand).initComptime(.{
    .{ "help", ParsedCommand{ .command = .help } },
    .{ "init", ParsedCommand{ .command = .init } },
    .{ "doctor", ParsedCommand{ .command = .doctor } },
    .{ "janitor", ParsedCommand{ .command = .prune } },
    .{ "prune", ParsedCommand{ .command = .prune } },
    .{ "setup", ParsedCommand{ .command = .setup } },
    .{ "provide", ParsedCommand{ .command = .providers_check, .pending_sub = "" } },
    .{ "providers", ParsedCommand{ .command = .providers_check, .pending_sub = "" } },
    .{ "run", ParsedCommand{ .command = .run } },
    .{ "sessions", ParsedCommand{ .command = .sessions } },
    .{ "history", ParsedCommand{ .command = .sessions } },
    // `session` alone is not a listing (`sessions` is); export is mandatory.
    .{ "session", ParsedCommand{ .command = .session_export, .pending_sub = "export" } },
    .{ "tools", ParsedCommand{ .command = .tools_list, .pending_sub = "list" } },
    .{ "eval", ParsedCommand{ .command = .eval } },
    .{ "improve-self", ParsedCommand{ .command = .improve_self } },
    .{ "revert", ParsedCommand{ .command = .revert } },
    .{ "git", ParsedCommand{ .command = .git } },
    .{ "mcp", ParsedCommand{ .command = .mcp } },
    .{ "goal", ParsedCommand{ .command = .goal } },
    .{ "notify", ParsedCommand{ .command = .notify } },
    .{ "chat", ParsedCommand{ .command = .chat, .pending_sub = "" } },
    .{ "stats", ParsedCommand{ .command = .stats } },
    .{ "phonebook", ParsedCommand{ .command = .phonebook } },
    .{ "serve", ParsedCommand{ .command = .serve } },
    .{ "graph", ParsedCommand{ .command = .graph } },
    .{ "autolearn", ParsedCommand{ .command = .autolearn } },
    .{ "repl", ParsedCommand{ .command = .repl } },
    // Compatibility alias from when the vaxis REPL was a separate command.
    .{ "repl-vaxis", ParsedCommand{ .command = .repl } },
    .{ "autoresearch", ParsedCommand{ .command = .autoresearch } },
    .{ "arena", ParsedCommand{ .command = .arena } },
    .{ "compare", ParsedCommand{ .command = .compare } },
    .{ "gate", ParsedCommand{ .command = .gate } },
    .{ "plugins", ParsedCommand{ .command = .plugins } },
    .{ "plugin", ParsedCommand{ .command = .plugins } },
    .{ "workflow", ParsedCommand{ .command = .workflow } },
    .{ "workflows", ParsedCommand{ .command = .workflow } },
    .{ "schedule", ParsedCommand{ .command = .schedule } },
    .{ "version", ParsedCommand{ .command = .version } },
});
