...
/// Test blocks are excluded: a test printing intermediate values is normal.
pub fn debugPrintGate(gpa: std.mem.Allocator, io: std.Io, dir: std.Io.Dir, files: []const []const u8, old_texts: []const []const u8, new_texts: []const []const u8) !GateResult {
    if (files.len != old_texts.len or files.len != new_texts.len)
        return .{ .ok = true, .label = "debug-print" };
    const needle = "std.debug.print";
    for (files, old_texts, new_texts) |f, old, new| {
        if (!std.mem.endsWith(u8, f, ".zig")) continue;
        // Only flag when the NEW text introduces the call (not already present in old).
        if (std.mem.indexOf(u8, new, needle) == null) continue;
        if (std.mem.indexOf(u8, old, needle) != null) continue;
        // Read the file to build the post-edit content and check whether the
        // call site is inside a `test` block (which is fine).
        const content = dir.readFileAlloc(io, f, gpa, .limited(4 << 20)) catch continue;
        defer gpa.free(content);
        const pos = std.mem.indexOf(u8, content, old) orelse continue;
        const after_buf = gpa.alloc(u8, content.len - old.len + new.len) catch continue;
        defer gpa.free(after_buf);
        @memcpy(after_buf[0..pos], content[0..pos]);
        @memcpy(after_buf[pos..][0..new.len], new);
        @memcpy(after_buf[pos + new.len ..], content[pos + old.len ..]);
        // Scan for std.debug.print outside test blocks.
        if (hasDebugPrintOutsideTests(after_buf, needle))
            return .{ .ok = false, .label = "debug-print", .detail = f };
    }
    return .{ .ok = true, .label = "debug-print" };
}

fn hasDebugPrintOutsideTests(src: []const u8, needle: []const u8) bool {
    // Walk each occurrence of the needle; for each one decide whether it
    // sits inside a `test "..." { ... }` block by counting braces backward
    // from a preceding `test ` keyword.
    var offset: usize = 0;
    while (offset < src.len) {
        const found = std.mem.indexOfPos(u8, src, offset, needle) orelse break;
        if (!isInsideTestBlock(src, found)) return true;
        offset = found + needle.len;
    }
    return false;
}

fn isInsideTestBlock(src: []const u8, pos: usize) bool {
    // Walk backwards from pos looking for the nearest top-level `test `.
    // A `test ` at brace-depth 0 (relative to pos) means pos is inside it.
    var depth: i64 = 0;
    var i: usize = pos;
    while (i > 0) {
        i -= 1;
        switch (src[i]) {
            ...
            '{' => {
                depth -= 1;
                if (depth < 0) {
                    // We exited the enclosing block; check if it is a test.
                    if (i >= 5 and std.mem.eql(u8, src[i -| 5..][0..5], "test ")) return false;
                    // Scan backwards past whitespace and a string to find `test`.
                    var j = i;
                    while (j > 0 and (src[j - 1] == ' ' or src[j - 1] == '\n' or src[j - 1] == '"')) j -= 1;
                    if (j >= 4 and std.mem.eql(u8, src[j - 4 .. j], "test")) return true;
                    return false;
                }
            },
            else => {},
...
test "debugPrintGate rejects std.debug.print in production code" {
    const gpa = std.testing.allocator;
    var threaded = std.Io.Threaded.init(gpa, .{});
    defer threaded.deinit();
    const io = threaded.io();
    ...
    // New code introduces std.debug.print in production: fail
    try tmp.dir.writeFile(io, .{ .sub_path = "a.zig", .data = "const x = 1;\n" });
    const bad = try debugPrintGate(gpa, io, tmp.dir, &.{"a.zig"}, &.{"const x = 1;"}, &.{"const x = 1;\nstd.debug.print(\"hi\", .{});"});
    try std.testing.expect(!bad.ok);
    try std.testing.expectEqualStrings("debug-print", bad.label);

    // Already present in old text: pass (not newly introduced)
    try tmp.dir.writeFile(io, .{ .sub_path = "b.zig", .data = "std.debug.print(\"old\", .{});\n" });
    const existing = try debugPrintGate(gpa, io, tmp.dir, &.{"b.zig"}, &.{"std.debug.print(\"old\", .{});"}, &.{"std.debug.print(\"new\", .{});"});
    try std.testing.expect(existing.ok);

    // Non-zig files: pass
    const json_ok = try debugPrintGate(gpa, io, tmp.dir, &.{"config.json"}, &.{"old"}, &.{"std.debug.print"});
    try std.testing.expect(json_ok.ok);

    // No changed files: pass
    const empty = try debugPrintGate(gpa, io, tmp.dir, &.{}, &.{}, &.{});
    try std.testing.expect(empty.ok);
}
