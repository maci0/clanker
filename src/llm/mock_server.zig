...
pub const MockServer = struct {
    ...
    pub fn start(io: std.Io, gpa: std.mem.Allocator, mode: Mode) !*MockServer {
        ...
        for (0..64) |_| {
            ...
            server = std.Io.net.IpAddress.listen(&addr, io, .{}) catch {
                std.debug.print("[mock] listen err, retry\n", .{});
                continue;
            };
            std.debug.print("[mock] listened port {d}\n", .{port});
            break;
        }
        if (server == null) return error.CannotBindMockPort;

        std.debug.print("[mock] start: create\n", .{});
    ...
    pub fn stop(self: *MockServer) void {
        ...
        self.server.socket.close(self.io); // unblocks accept()
        self.thread.join();
    ...
    fn serve(self: *MockServer) void {
        std.debug.print("[mock] serve thread started\n", .{});
        while (!self.stop_flag.load(.acquire)) {
            const stream = self.server.accept(self.io) catch |err| {
                std.debug.print("[mock] accept err {s}\n", .{@errorName(err)});
                break;
            };
            std.debug.print("[mock] accepted\n", .{});
            self.handle(stream);
            std.debug.print("[mock] handled, closing\n", .{});
            stream.close(self.io);
        }
        std.debug.print("[mock] serve thread exiting\n", .{});
    }

    fn handle(self: *MockServer, stream: std.Io.net.Stream) void {
        std.debug.print("[mock] handle start\n", .{});
    ...
    fn respond(self: *MockServer, stream: std.Io.net.Stream) void {
        const pair = switch (self.mode) {
            ...
            , @as(u16, 401), @as([]const u8, "Unauthorized"),
            },
        };
        const body = pair[0];
        const status = pair[1];
        const reason = pair[2];
        std.debug.print("[mock] responding\n", .{});
