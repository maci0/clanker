...
const Model = struct {
    ...
    /// The saved conversation's title, kept from load so a `renameSession`
    /// (or an explicit title) survives a resume; empty means \"derive from the
    /// opening user message at save time\" (the default cmdRun title rule).
