...
if [ -n "$INSTRUCTION" ]; then
  ...
  INSTRUCTION="Review the clanker codebase (src/, tool-src/, tools.d/, evals/, skills/) for bugs, dead code, and token-efficiency improvements. Use your search_code and git tools to investigate, then propose and implement the highest-impact fixes. Every change must pass the full gate (zig build, zig build test, zig build tools, zig fmt, lint). Persist learnings with write_note."
fi
