...
- **REPL with streaming** – interactive session with live token output, plus slash commands (`/help`, `/model`, `/workflows`, `/workflow`, `/sessions`, `/graph`, `/status`, `/plugins`, `/theme`, `/preset`, `/effort`, `/goal`, `/autoresearch`, `/arena`, `/compare`) with Tab-complete; some run in-process, the rest dispatch to an internal WASM tool
...
| `janitor [--yes]` | Sweep up what old runs left behind: staging copies, old run graphs and improve logs, and compare-and-swap lock files unused for 12h (also `clanker prune`) |
