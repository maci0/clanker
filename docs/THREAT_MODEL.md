...
| R6 | **DoS: connection limit 64** (`max_connection_threads`, `src/cli.zig:7752`, enforced `:7782`; `/health/ready` reports `saturated`, `src/cli.zig:9274-9288`), request bodies capped at `max_body_bytes` +64 KiB slack (`src/cli.zig:7959`), images capped 4 MB × 4 (`docs/README.md:1604`). Saturation refusals are recorded in metrics and logs since 08-21 (`respondSaturated`, `src/cli.zig:7814-7851`). The dedicated proxy listener reserves `proxy.webui_reserved_slots` so it cannot starve the web UI (`src/cli.zig:7789`); the proxy's upstream deadlines default to 300 s first byte / 60 s idle (`src/serve/proxy.zig:29-30`, knobs `src/config.zig:1000-1003`). The mesh join handshake holds its own bounded thread pool (`max_inbound_conns`, `src/serve/mesh_net.zig:38`, enforced `:459-468`). Still no rate limiting per-route; an unauthenticated surface means any local process can hold all 64 slots. | Medium | Medium | Loopback-only default keeps this local. |
...
|-------------|-------|---------------|-------------|
| HTTP server (web UI + every `/api/*` route, health, metrics, A2A, `/proxy/v1`) | `clanker serve`: accept loop `src/cli.zig:7603` (proxy listener `:7639`, spawn `:7656`), per-connection `serveConnection` `src/cli.zig:7778`; route table `docs/README.md:1457+` | `127.0.0.1:17921` (`--host` widens; one socket, `docs/README.md:1457`) | **None**; Host allowlist + Origin check only |
...
| Mesh TCP listener | `src/serve/mesh_net.zig` (`acceptLoop` `:453`; inbound cap `:38`, `:459-468`) | `127.0.0.1:7420` (`src/config.zig:938`) | Admission allowlist/prompt/open (`src/peers/mesh.zig:114,123`); JOIN handshake bounded by frame cap + read timeout |
...
| REPL `!cmd` shell escape | `docs/README.md:812` | Deliberate: interactive user shell |
...
- `clanker schedule run-due` invoked by system cron (`src/schedule/`); nothing fires on its own
  ...
  execution.
- **Opt-in state-backup timers** (new surface since 08-25): user-level systemd units installed
  by `scripts/install-state-backup.sh` — `clanker-state-backup.timer` runs
  ...
  goals, plugins) to a sibling `backups/` root outside the checkout and refuses to run when
  that root would land inside the repo (`scripts/backup-state.sh:17-31`). Threat shape: same
  trust level as cron (runs as the operator's user), but it widens where transcripts live —
  see T5/T6 and the assets table.
...
- SSE event stream `GET /api/events`, long-lived, Origin-gated (`src/cli.zig:8220-8224`).
...
- Saved sessions loaded from `state/sessions/<id>.db` (WAL-mode SQLite, one db per
  conversation, `src/agent/session.zig:32-34`, `src/util/sqlite.zig`), goal records from
...
- No admin/debug ports beyond the ones listed; `/health/live`, `/health/ready` and
  `/api/metrics` ride the same socket (`src/cli.zig:8072`, `:8183`, `:8117`/`:8187`).
...
| T5 | **Disk state → process** | `state/sessions/<id>.db`, `state/goals.json`, `state/models-dev.json`, `state/board*.json`, `state/plugins.json` + `plugin_config.json` | Parsed with explicit bounds (1 MiB arena reads via `ck_fs_read_range`); `.env` refused by `safeJoin`; symlinked components refused by `safeJoinSecure` (ADR 0017) |
...
- operator CLI → scheduled execution: `clanker schedule` writes `state/schedule.json`;
  ...
  `scripts/install-state-backup.sh` enables user-level systemd units that run
  `scripts/backup-state.sh` as the operator.
...
- **Information disclosure**: GET endpoints expose logs (`/api/logs`), sessions
  ...
  `docs/README.md:1457+`).
- **DoS**: 64 connection slots (`src/cli.zig:7752`, enforced `:7782`); body cap
  (`src/cli.zig:7959`); no per-route rate limit; `POST /api/run` holds a slot for the whole
  run; provider hangups are bounded only by `agent.request_timeout_ms` /
  `agent.stream_idle_timeout_ms` (the HTTP client itself has no read timeout). Saturation 503s
  are counted under `errors_total`, logged, and visible in `/health/ready` as `"saturated"`
  (`respondSaturated`, `src/cli.zig:7814-7851`; readiness `:9274-9288`); until 08-21 the one
  load condition operators look for was the one the server did not record. The dedicated proxy
  listener reserves `proxy.webui_reserved_slots` so it cannot starve the web UI
  (`src/cli.zig:7789`); the proxy's own upstream deadlines default to 300 s/60 s
  (`src/serve/proxy.zig:29-30`).
...
- **Information disclosure**: guest HTTP responses expose only allowlisted headers
  (`exposed_response_headers`, `src/sandbox/host.zig:3125`, ADR 0049), `Set-Cookie`,
...
| M12 | Body caps: `max_body_bytes` +64 KiB slack (`src/cli.zig:7959`); images 4 MB × 4; connection limit 64 (`:7752`, enforced `:7782`); proxy reserved slots (`:7789`) + upstream deadlines (`src/serve/proxy.zig:29-30`); saturation 503 counted/logged (`:7814-7851`); mesh join-handshake connection cap (`src/serve/mesh_net.zig:38`, `:459-468`) | see left | R6, T3 DoS |
| M13 | Mesh admission (allowlist/prompt/open) + loopback default | `src/peers/mesh.zig:114,123`, `src/config.zig:938` | R4 (partial, name-match, no credential) |
| M14 | Peers are outbound-only; nothing listens for peer traffic | `docs/README.md:1557` | T3 reach |
| M15 | Guest-visible HTTP response headers are an allowlist, lowercased, value-capped | `exposed_response_headers` `src/sandbox/host.zig:3125`, ADR 0049 | T2 information disclosure |
...
2. **Proxy token off by default** (R2): M4 exists but is opt-in; the warning at
   `src/cli.zig:7629` is the only guard when unset.
...
- Starter created 2026-08-19; re-verified 2026-08-20; passes 2026-08-25 and 2026-08-26.
  Sections 2-4 are partial: internet-facing (T1, T2) and authentication (T1) boundaries are
  complete; the mesh/peer, disk-state, and secrets boundaries are summarized and need a
  dedicated pass. The backup-timer surface (section 1, Scheduled) was added 08-26 and is
  noted but not yet threat-enumerated per STRIDE.
...
- Audit trail: `state/sessions/<id>.db` (transcripts), `state/token_stats.jsonl`, `state/logs/`
  ...
  root extends how long transcripts outlive their deletion from `state/`. Structure is owned
  by o11y-review; noted here that security events (a failed proxy auth, a refused host or
  Origin, a refused sandbox path, a saturation 503) surface in logs and, since 08-21,
  saturation is also in `/api/metrics` (`src/cli.zig:7814-7851`), but there is no dedicated
  security event log.
