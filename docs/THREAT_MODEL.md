...
Last reviewed: 2026-08-19 (starter created from `src/`, `docs/README.md`).
...
|---|------|--------|------------|-------|
| R1 | **Control plane has no authentication.** `clanker serve` exposes the full agent (`/api/run` runs tasks, tools exec and write, `/api/ask` answers write confirmations) to *anyone who can reach the port* — this is stated, not hidden: `docs/README.md:1349`. Mitigated only by loopback bind default, the Host/Origin guards, and a firewall. | Critical | High (any local process, any LAN client once `--host` widened) | The single biggest exposure; everything else hangs off it. |
| R2 | **Proxy credential spending.** `/proxy/v1` (OpenAI/Anthropic compat) lets a caller spend any configured provider's credentials, incl. Vertex. `proxy_token_env` is **optional** and absent by default; only a stderr warning fires when it is unset on a non-loopback bind (`src/cli.zig:6949`, `src/proxy_main.zig:93`). | High (financial: token spend, data exfil via prompt) | High when exposed | Same socket as the control plane by default (`docs/README.md:1352`). |
| R3 | **Prompt injection through LLM responses.** Provider output is untrusted input to the agent loop; retrieved documents, memory hits, and web results are untrusted text the model is told never to execute (`src/agent/system_prompt.zig:632`, self-authored notice at `:78`, tests at `:668`). Containment is the sandbox, not the prompt. | High (tool misuse within sandbox policy) | Certain (inherent to an agent harness) | The sandbox is the trust boundary that makes this survivable; see M3. |
| R4 | **Mesh join without credential.** Mesh admission is allowlist-by-name, prompt, or open (`src/peers/mesh.zig:121-136`); the wire carries no authentication beyond the admission handshake and no encryption (plain TCP, `src/serve/mesh_net.zig:449`). Default bind is loopback `127.0.0.1:7420` (`src/config.zig:743-744`). | Medium (chat/fan-out spoofing, membership) | Medium (needs LAN reach or misconfig) | Off by default (`modules.mesh`). |
...
| R6 | **DoS: connection limit 64** (`max_connection_threads`, `src/cli.zig:7076`; `/health/ready` "saturated", `src/cli.zig:8466`), request bodies capped at `max_body_bytes` (`src/cli.zig:7206`), images capped 4 MB × 4 (`docs/README.md:1397`). No rate limiting per-route; authenticated-less surface means any local process can hold all 64 slots. | Medium | Medium | Loopback-only default keeps this local. |
...
|-------------|-------|---------------|-------------|
| HTTP server (web UI + every `/api/*` route, health, metrics, A2A, `/proxy/v1`) | `clanker serve`, loop in `src/cli.zig:7200-7297`; route table `docs/README.md:1271-1341` | `127.0.0.1:17921` (`--host` widens; one socket, `docs/README.md:1349-1352`) | **None**; Host allowlist + Origin check only |
| Proxy listener (dedicated) | `--proxy-port`; `/v1` at root, no `/api/*` (`docs/README.md:1352`) | `127.0.0.1:17922` standalone (`src/proxy_main.zig`) | Optional `proxy_token_env` (`src/cli.zig:7256`) |
| Mesh TCP listener | `src/serve/mesh_net.zig:444-449` | `127.0.0.1:7420` (`src/config.zig:743`) | Admission allowlist/prompt/open (`mesh.Admit`, `src/peers/mesh.zig:121-136`) |
| Outbound peer HTTP (`POST /api/chat/message`, notify) | `src/peers/chatrooms.zig` fan-out; `src/peers/command.zig` | — | Peers are *outbound* URLs, never listeners (`docs/README.md:1352`) |
...
- HTTP request bodies (JSON), headers (`Host`, `Origin`, `Content-Type`), query strings,
  resource ids (`requestPath` strips query before id, `docs/README.md:1343`).
- `/api/run` `images` (base64, capped 4 MB × 4, `docs/README.md:1397`).
- Chatroom messages fanned in from peers (`POST /api/chat/message`, `src/peers/chatrooms.zig`).
- Mesh wire frames (length-prefixed, `src/serve/mesh_net.zig:320`), JOIN name/id, seeds.
...
|---|----------|-----------|--------------------------|
| T1 | **Client → HTTP control plane** | Browser/SDK/curl → `/api/*`, `/proxy/v1` | No authn. Host header checked on *every* request (`unexpectedHost`, `src/cli.zig:15522`, enforced `src/cli.zig:7249`); `Origin` checked on non-GET as CSRF (`crossOriginRequest`, `src/cli.zig:15502`, enforced `src/cli.zig:7276`); body capped (`src/cli.zig:7206`). Loopback bind is the real control |
| T2 | **HTTP control plane → agent/tools** | `/api/run` task text → agent loop → sandboxed tools | Descriptor policy: `fs_prefixes`, `env_allow`, `network_allow`, `exec_allow` (`src/sandbox/host.zig:245-318`); privileged `ck_*` channels check `tool_self_name` (`src/sandbox/host.zig`, 32 sites) |
| T3 | **Peer/mesh → local state** | `POST /api/chat/message`, mesh CHAT frames → `state/chatrooms.jsonl`, `state/notifications.jsonl` | Chat fan-out via sandboxed `peers` tool (`chat_fanout`, `network_from_config`); mesh admission handshake (`mesh_net.zig:383-401`); no wire crypto |
...
| T6 | **Secrets → code** | Provider keys via `api_key_env` (`src/config.zig:1205-1246`), `[serve] proxy_token_env`, Vertex service-account JWT minting (`src/llm/vertex_token.zig`) | Keys live in `config.toml`/`config.local.toml`/env; guest access gated by `env_allow` + named `ck_getenv` (`docs/README.md` sandbox section); proxy forwards creds only on `/v1/*` (`src/serve/proxy.zig:248`) |
...
- sandboxed tool → unsandboxed kernel subprocess: `ck_kernel` requires `kernel.enabled`
  (`src/config.zig:556`); `ck_docker` requires descriptor grant; both opt-in (ADR 0010).
...
- **Spoofing**: none — no authn. Any process on the host (or LAN once `--host` widened) is the
  operator. Accepted by design (`docs/README.md:1349`).
- **Tampering**: cross-site POST refused by Origin check (`src/cli.zig:7276`) — but only for
...
- **DoS**: 64 connection slots (`src/cli.zig:7076`); body cap (`src/cli.zig:7206`); no
  ...
  itself has no read timeout, `docs/README.md` agent section).
- **Elevation**: `/api/ask` answers `confirm` events — a same-origin script or local client
  that can already reach the port can also confirm writes (`docs/README.md:1410`).

### T2 (HTTP → agent/tools)

- **Elevation**: guest → host via `ck_*`. Mitigated by descriptor policy + `tool_self_name`
  checks on privileged channels (M3). History: symlink escape refused by `safeJoinSecure`
...
- **Spoofing**: mesh admission by self-asserted name (`matchesSeed`, `src/peers/mesh.zig:132`)
...
- **Tampering / Elevation (prompt injection)**: provider output and retrieved text are
  ...
  (`src/agent/system_prompt.zig:78,632`); fence markers inside retrieved text are neutralized so
...
|---|---------|----------------|--------|
| M1 | Loopback bind by default; exactly one socket; `--host` opt-in widening | `src/cli.zig:2079`, `docs/README.md:1349-1352` | R1, R2, R6 (network reach) |
| M2 | Host allowlist (DNS-rebinding defense) on every request, incl. GET | `unexpectedHost` `src/cli.zig:15522`, enforced `src/cli.zig:7249`; tests `src/cli.zig:15578` | R1 (rebinding) |
| M3 | Origin check on non-GET (CSRF) | `crossOriginRequest` `src/cli.zig:15502`, enforced `src/cli.zig:7276` | R1 (cross-site) |
| M4 | Optional proxy token | `proxy.authorize` `src/serve/proxy.zig:53`; wiring `src/cli.zig:7256`, `src/proxy_main.zig:210`; warn when unset on non-loopback `src/cli.zig:6949` | R2 (partial — off by default) |
| M5 | WASM sandbox: descriptor policy (`fs_prefixes`/`env_allow`/`network_allow`/`exec_allow`), size caps | `src/sandbox/host.zig:245-318`, `:3` | R3, R5, T2, T3 |
...
| M11 | Prompt-injection posture: untrusted retrieved text fenced, model told never to execute it | `src/agent/system_prompt.zig:78,632` | R3 (advisory; sandbox enforces) |
| M12 | Body caps: `max_body_bytes` +64 KiB slack; images 4 MB × 4; connection limit 64 | `src/cli.zig:7206`, `docs/README.md:1397`, `src/cli.zig:7076` | R6 |
| M13 | Mesh admission (allowlist/prompt/open) + loopback default | `src/peers/mesh.zig:121-136`, `src/config.zig:743` | R4 (partial — name-match, no credential) |
| M14 | Peers are outbound-only; nothing listens for peer traffic | `docs/README.md:1352` | T3 reach |
...
2. **Proxy token off by default** (R2) — M4 exists but is opt-in; the warning at
   `src/cli.zig:6949` is the only guard when unset.
...
- The **Host/Origin guard** (`src/cli.zig:7249-7279`) is the entire CSRF/rebinding defense
...
- **Credential spending via proxy**: `POST /proxy/v1/chat/completions` with any body spends the
  ...
  (`src/serve/proxy.zig:53`, `src/cli.zig:7256`). Enabling `--host 0.0.0.0` without a token
  makes this LAN-wide (the warning at `src/cli.zig:6949` fires, nothing stops it).
...
- **Write confirmation bypass**: `/api/ask` answers `confirm` events with a byte-exact option
  check (`docs/README.md:1410`); a client that already reaches the port can answer "allow"
...
- **Transcript scraping**: `GET /api/sessions` + `/api/sessions/<id>` and `/api/logs` are
  unauthenticated reads of conversation and log content (route table `docs/README.md:1290,1339`).
...
- Starter created 2026-08-19; no prior model existed. Sections 2-4 are partial: internet-facing
  (T1, T2) and authentication (T1) boundaries are complete; the mesh/peer, disk-state, and
  secrets boundaries are summarized and need a dedicated pass.
