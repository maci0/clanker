...
- **What failed:** ToolWorker built its own Sandbox literal and omitted the new `session` field added by the ck_session seam, so session_search was denied ck_session on the parallel path only, scored 0.00 in the improve-self capability eval, and every staged tree was rejected.
...
- **Resolution:** Resolved on 2026-08-20. ToolWorker now delegates to host.sandboxFor, granting the session channel on the parallel path; full gate green.

## Status

Resolved on 2026-08-20. ToolWorker now delegates to host.sandboxFor, granting the session channel on the parallel path; full gate green.
`host.sandboxFor` (the documented single place a tool's sandbox policy is
assembled) instead of a hand-rolled `Sandbox` literal, so the `session` grant
from the descriptor reaches the parallel path. `zig build`, `zig build tools`,
`zig build test` and `zig fmt --check` are green.
...
```
[WARN] improve capability evals: 3 case(s) failed; retrying only those
...
builds its own sandbox with `session = true` and never exercises the worker's
literal.
