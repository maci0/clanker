...
effects, save a durable goal without starting work, and execute a raw request
immediately. Coupling them makes a headless execution ask unnecessary
...
`add_goal` persists only; `goal` executes only. Each has a direct CLI and TUI
entry point. `run --goal <id>` is the separate execution path for an existing
record, and the web board creates through `add_goal`.
