...
- [ ] `@embedFile` for small comptime assets is the existing, correct
      pattern (`ui/webui.zig` embeds `app/index.html` and the rest of the
      web UI); don't flag it, but do flag a new `@embedFile` of something
      large enough to bloat the guest module that doesn't need it.
