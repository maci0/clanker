...
|---|---|---|---|
| 1 | Bordered tool-call cards (name, args preview, truncated output, elapsed time) | `src/tui/transcript.zig` (`printCallCard`/`printResultBody`/`printCardFooter`), wired via `replToolCall`/`replToolResults`/`replToolResult` in `src/cli.zig`, fed by the additive `Agent.on_tool_results` hook (`src/agent/loop.zig`) | unit: `transcript.zig` (7 card tests). Not yet in `demo.zig` — needs a task that triggers a real tool call, which the pty suite deliberately avoids to not spend API credits (see demo.zig's module doc comment). |
