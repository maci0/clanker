...
- `zig build` EXIT 0, `zig build tools` EXIT 0, `zig build test --summary all` 368/369 pass (1 skipped) — `clanker gate` green at `4405546`; `zig fmt --check src/cli.zig tools/zig/webui.zig` EXIT 0.
...
- `core/attachments.js` — `pendingImages/addImageFile/renderAttachments/max_image_bytes` extracted (live `pendingImages` array, `el`/`icon`/`fmtBytes` injected); `app.js` was 4464 lines pre-logs.
- `core/status.js` — `renderStatusInto(status, els)` extracted (returns `{instanceName, knownPeers}` and owns the peer-link table); `app.js` was 4506 lines pre-attachments.
- `core/usage.js` — `renderUsageTable/usageColumns/usageName/usageRow` extracted (pure helpers, `modelLabel`/`fmt`/`UI`/`T` injected); `app.js` was 4546 lines pre-status.
- `core/dialog.js` — `textPrompt/finishTextPrompt/bindDialog + SHORTCUTS` extracted (own module + `els`/`open`/`close` binding, shortcut table populated there); `app.js` was 4589 lines pre-usage.
- `core/composer.js` — `loadPrompts/savePrompts/promptQuery/autoGrow/contextLabel` extracted as a real module (pure, tested via `node --check`); `app.js` was 4682 lines pre-dialog.
- `showToast` delegates to `core/ui.js:toast`; `llm`-phase lamp breathes alongside `ask` (prior turn).
