...
- `tools/zig/webui/lib/board.js` — `BOARD_COLUMNS, boardActionLine` extracted from `app.js` (pure, no DOM, bridged as `window.ckBoard`)
...
- `tools/zig/webui.zig` — embed + comptime `encodedLen` guard + `assetFor` for all **14** webui assets (now incl. `core/stream.js` alongside `lib/board.js`).
- `src/cli.zig` — `is_webui` exact paths + `handleWebuiAsset` `RenderCache/GzipCache` vars for each new module (now 14 routes); `stream` alongside `board/graph/markdown`.
- `tools/zig/webui/index.html` — rail `Watch > Fleet` tab, `#view-fleet` with roster/DMs/runs/detail, script order `van-boot → van-ui defer → core/utils → core/icons → core/vendor → core/stream → core/theme → lib/markdown → lib/graph → lib/board → features/fleet → app.js defer` (module+defer document order).

### App.js dedupe (monolith shrink)
- `app.js` now bridges instead of duplicating: `fmtInt/fmtMs/fmtCost/fmtDeadline/formatChatTime → window.ckUtil`, `T/bind/skeletonRows/setTurnPhase/icon/UI → window.ckUi/ckIcons`, `vendor/lifecycle → window.vendorLoads/ckBoard`, `THEMES/loadTheme/applyTheme → window.ckTheme`, `BOARD_COLUMNS/boardActionLine → window.ckBoard`, `graph helpers → window.ckGraph`, `makeLineSplitter → window.ckStream`. Current: `app.js` **4893** lines (from 5511 at start; −618 total this session, −25 this slice).
...
- `lib.out_cap = 2MiB` comptime guard passes (largest encoded `app.js` ~203KB; new modules 1–9KB each; headroom ~1.8MB; 14 assets).
...
- `out_cap` per-file: all `ok` (max ~203KB well under 2MiB; 14 assets).
