...
- `app.js` now bridges instead of duplicating: `fmtInt/fmtMs/fmtCost/fmtDeadline/formatChatTime → window.ckUtil`, `T/bind/skeletonRows/setTurnPhase/icon/UI → window.ckUi/ckIcons`, `vendor/lifecycle → window.vendorLoads/ckBoard`, `THEMES/loadTheme/applyTheme → window.ckTheme`, `BOARD_COLUMNS/boardActionLine/doneColumn/blockers/dueState → window.ckBoard`, `dmRoom/dmSafeName/dmPartner/isDm/clankerMark → window.ckChat`, `graph helpers → window.ckGraph`, `makeLineSplitter → window.ckStream`. Current: `app.js` **4833** lines (from 5511 at start; −678 total this session, −43 this slice).
