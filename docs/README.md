...
The REPL also has an opt-in mascot (`--mascot`, or `[tui] mascot` in config; off by default): a small robot animated from an eleven-frame run cycle, which either travels with what you type, loops across the screen, runs on the spot above the composer, or runs inside the composer itself. Three renderers share one set of artwork, picked in order from what the terminal actually answered to a capability query rather than from `$TERM`: kitty graphics (Ghostty, kitty, iTerm2), then sixel where the terminal proved sixel support and not kitty, then unicode half-blocks quantized to the artwork's own seven colours (`src/tui/mascot.zig`, frames generated by `src/tui/mascot/gen_frames.py`). Kitty and sixel show the same artwork in the same number of cells; half-blocks approximate it with two colour samples per cell. See [ADR 0013](adrs/0013-sixel-precedes-unicode-mascot-fallback.md) for the ordering and [PRD 0036](prds/0036-sixel-mascot-rendering.md) for the sixel path. It is decoration, not a status indicator: every mode but `input` reserves 1 to 10 transcript rows depending on `mascot_size`, and a terminal too small for the chosen size gets no mascot rather than a clipped one. `input` reserves none, and at its default `mini` size leaves the composer the height it has with the mascot off.
...
| `scripts/verify.sh` | Mirror the full CI verify job locally: shellcheck, oxlint, `tools/ts` audit + rebuild-and-diff, SBOM generation, Python syntax check, then `zig build` + `clanker gate` + `zig build e2e`. The checks CI runs that `clanker gate` does not (shellcheck, oxlint, Python, SBOM) live only in `.github/workflows/ci.yml` otherwise |
...
| `/api/janitor` | GET | How much litter (staging copies, run graphs, improve logs, aged compare-and-swap locks, spilled tool results) is reclaimable; read-only, never deletes |
...
`reasoning_effort` is an optional per-run reasoning-effort pin, the request-shaped equivalent of the CLI's `--reasoning-effort` and the TUI's `/effort` picker (the webui composer's Advanced fold sends it). One of `"none"`/`"low"`/`"medium"`/`"high"`/`"max"`: it lands on `agent.reasoning_effort` for this run only, so it beats the `auto_thinking` classifier, the per-model setting and the sampling profile on every turn. Empty or omitted leaves those lower layers in charge; an unknown value is refused with a 400 rather than silently ignored.
...
`error` events (`{"type":"error","message":"..."}`) can appear instead of `done` if the run fails mid-stream. A client must buffer on `\n` and only treat a *complete* line starting with `0x01` as an event; a naive per-chunk check can split an event across two reads. The web UI's `makeLineSplitter` in [`ui/app/core/stream.js`](../ui/app/core/stream.js) is the reference implementation.
