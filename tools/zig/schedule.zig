...
fn doAdd(req: std.json.Value, out: *lib.Out) !void {
    ...
    const now: i64 = @trunc(lib.nowSeconds());
    if (cron_text.len > logic.max_cron_spec_bytes)
        return lib.fail(out, "cron spec is too long");
    if (!logic.parses(cron_text))
        return lib.fail(out, "cron spec is not a valid five-field expression");
    if (logic.firstFire(cron_text, now, tz) == null)
        return lib.fail(out, "cron spec parses but never comes around");
