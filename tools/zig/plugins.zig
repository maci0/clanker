...
fn listJson(out: *lib.Out, alloc: std.mem.Allocator, plugins: []const Plugin) !void {
    ...
    try text.appendSlice(alloc, "\nToggle with `clanker plugins on|off <name>` or `/plugins on|off <name>` in the REPL. Core tools back the REPL and HTTP routes and stay on; transforms rewrite other tools' input or output.\n");
